...
struct AutoencoderBackwardCache {
    ...
    // Fix: Store per-timestep output projection caches
    std::vector<LinearCache> output_proj_caches; // Store all timestep caches
    LinearCache output_proj_cache; // Keep single cache for backward compatibility
...
private:
    ...
    decode_autoregressive_complete(const MatrixXf& latent, const std::vector<MatrixXf>& dt_decode_seq,
        ...
        std::vector<MatrixXf> decoder_h_states_all = decoder_h_states;
        ...
        for (int t = 0; t < T_decode; ++t) {
            ...
            auto [step_hidden, step_final_h, step_cache] = decoder_rnn->forward(input_seq, dt_seq, mask_seq, decoder_h_states_all);
            step_caches.push_back(step_cache);
            decoder_h_states_all = step_final_h;

            // Project to output
            auto [step_output, output_cache] = output_proj->forward(step_hidden);
            outputs_all.push_back(step_output);

            // Fix: Store each timestep's output cache
        ...
        return {output_sequence, decoder_h_states_all, {step_caches, feedback_caches_all, output_caches_all}};
    }
...
void complete_usage_example() {
    ...
    for (int t = 0; t < 30; ++t) {
        ...
        target_t = MatrixXf(target_t.data(), 3 * B, 6); // Fix: Replace reshaped() with proper matrix constructor
