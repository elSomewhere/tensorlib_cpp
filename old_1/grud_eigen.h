...
public:
    ...
    VectorXf backward_gamma(const MatrixXf &d_gamma, const GammaCache &cache)
    {
        ...
        MatrixXf grad_clip_mask = ((cache.log_gamma_unclamped.array() >= cfg.min_log_gamma) &&
                                   (cache.log_gamma_unclamped.array() <= -1e-4f))
                .cast<float>()
                .matrix();
...
public:
    ...
    std::pair<MatrixXf, MatrixXf> backward(const MatrixXf &d_h_next, const CellFwdCache &cache)
    {
        ...
        V_h_candidate.backward(d_h_cand_arg, cache.hc_arg_Vdt_ic); // Grad for dt ignored
        ...
        V_z.backward(d_z_arg, cache.z_arg_Vdt_ic); // Grad for dt ignored
        ...
        V_r.backward(d_r_arg, cache.r_arg_Vdt_ic); // Grad for dt ignored
        ...
        return {d_x_raw, d_h_prev_total};
    }

    // NEW: Special backward method that exposes mask gradients (for use by mask projectors)
    std::tuple<MatrixXf, MatrixXf, MatrixXf> backward_with_mask_grad(const MatrixXf &d_h_next, const CellFwdCache &cache)
    {
        if (!cfg.enable_mask_learning) {
            // If mask learning is disabled, return zeros for mask gradient
            auto [d_x_raw, d_h_prev] = backward(d_h_next, cache);
            MatrixXf d_mask_zero = MatrixXf::Zero(d_x_raw.rows(), d_x_raw.cols());
            return {d_x_raw, d_h_prev, d_mask_zero};
        }
        ...
        MatrixXf d_x_tilde_from_Wh = W_h_candidate.backward(d_h_cand_arg, cache.hc_arg_Wx_ic);
        MatrixXf d_rh_decay_prod_from_Uh = U_h_candidate.backward(d_h_cand_arg, cache.hc_arg_Uh_ic);
        V_h_candidate.backward(d_h_cand_arg, cache.hc_arg_Vdt_ic);
        ...
        MatrixXf d_x_tilde_from_Wz = W_z.backward(d_z_arg, cache.z_arg_Wx_ic);
        MatrixXf d_h_decay_from_Uz = U_z.backward(d_z_arg, cache.z_arg_Uh_ic);
        V_z.backward(d_z_arg, cache.z_arg_Vdt_ic);

        MatrixXf d_r_arg = d_r_from_rh_prod.array() * d_sigmoid(cache.r_arg_val).array();

        MatrixXf d_x_tilde_from_Wr = W_r.backward(d_r_arg, cache.r_arg_Wx_ic);
        MatrixXf d_h_decay_from_Ur = U_r.backward(d_r_arg, cache.r_arg_Uh_ic);
        V_r.backward(d_r_arg, cache.r_arg_Vdt_ic);
        ...
        return {d_x_raw, d_h_prev_total, d_mask};
    }
...
public:
    ...
    std::pair<MatrixXf, MatrixXf> backward(const MatrixXf &d_h_output, const LayerFwdCache &cache)
    {
    ...
    std::tuple<MatrixXf, MatrixXf, std::optional<MatrixXf>> backward_with_mask_grad(
            const MatrixXf &d_h_output, const LayerFwdCache &cache)
    {
        ...
        if (layer_idx == 0 && cfg.enable_mask_learning) {
            auto [d_input, d_h_prev, d_mask] = cell->backward_with_mask_grad(d_h_before_ln, cache.cell_fwd_cache);
            return {d_input, d_h_prev, std::make_optional(d_mask)};
        } else {
            auto [d_input, d_h_prev] = cell->backward(d_h_before_ln, cache.cell_fwd_cache);
            return {d_input, d_h_prev, std::nullopt};
        }
    }
        ...
        for (int t = T_win - 1; t >= 0; --t)
        {
            ...
            for (int l_idx = num_layers - 1; l_idx >= 0; --l_idx)
            {
                if (cfg.enable_mask_learning && l_idx == 0) {
                    ...
                    auto [d_input_for_layer_l_at_t, d_h_prev_for_layer_l_at_t, d_mask_for_layer_l_at_t] =
                    ...
                    auto [d_input_for_layer_l_at_t, d_h_prev_for_layer_l_at_t] =
