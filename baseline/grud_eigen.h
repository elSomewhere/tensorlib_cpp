...
inline float stable_expm1(float x) {
    if (std::abs(x) < 1e-5f) {
        // Taylor series for small x: expm1(x) ≈ x + x²/2 + x³/6 + ...
        return x * (1.0f + x * (0.5f + x * (1.0f/6.0f + x * (1.0f/24.0f))));
    } else {
...
inline float stable_log1p(float x) {
    if (std::abs(x) < 1e-5f) {
        // Taylor series for small x: log1p(x) ≈ x - x²/2 + x³/3 - x⁴/4 + ...
        return x * (1.0f - x * (0.5f - x * (1.0f/3.0f - x * 0.25f)));
    } else {
        return std::log(1.0f + x);
    }
}

inline float stable_log1p_expm1(float y) {
    // PyTorch's log1p(expm1(y)) computation
    if (y < 1e-5f) {
        return y;  // For very small y, use the identity
    } else if (y > 20.0f) {
        return y;  // For large y, numerically stable
    } else {
        // For moderate y, compute exactly like PyTorch
        float expm1_y = stable_expm1(y);
        return stable_log1p(expm1_y);
    }
}

// FIXED: PyTorch-compatible softclip for VectorXf
VectorXf softclip(const VectorXf &x, float threshold)
{
    ...
    for (int i = 0; i < x.size(); ++i) {
        ...
        if (abs_xi <= threshold) {
            result(i) = xi;  // Unchanged in linear region
        } else {
            // PyTorch formula: threshold + log1p(expm1(|x| - threshold)) * sign(x)
            float y = abs_xi - threshold;
            float log1p_expm1_y = stable_log1p_expm1(y);
            float sign_xi = (xi >= 0.0f) ? 1.0f : -1.0f;
            result(i) = threshold + log1p_expm1_y * sign_xi;
        }
    }
...
// FIXED: PyTorch-compatible softclip for MatrixXf
MatrixXf softclip(const MatrixXf &x, float threshold)
{
    ...
    MatrixXf result(x.rows(), x.cols());

    for (int i = 0; i < x.rows(); ++i) {
        for (int j = 0; j < x.cols(); ++j) {
            float xij = x(i, j);
            float abs_xij = std::abs(xij);

            if (abs_xij <= threshold) {
                result(i, j) = xij;  // Unchanged in linear region
            } else {
                // PyTorch formula: threshold + log1p(expm1(|x| - threshold)) * sign(x)
                float y = abs_xij - threshold;
                float log1p_expm1_y = stable_log1p_expm1(y);
                float sign_xij = (xij >= 0.0f) ? 1.0f : -1.0f;
                result(i, j) = threshold + log1p_expm1_y * sign_xij;
            }
        }
    }
...
VectorXf d_softclip(const VectorXf &x, float threshold)
{
    ...
    for (int i = 0; i < x.size(); ++i) {
        float abs_xi = std::abs(x(i));

        if (abs_xi <= threshold) {
            result(i) = 1.0f;  // Linear region: derivative = 1
        } else {
            // In clipped region, the derivative is still 1 due to the mathematical identity
            // d/dx[threshold + log1p(expm1(|x| - threshold)) * sign(x)] = 1
            result(i) = 1.0f;
        }
    }
...
MatrixXf d_softclip(const MatrixXf &x, float threshold)
{
    ...
    // The derivative of softclip is always 1 everywhere!
...
public:
    ...
    struct RNNBackwardOutput {
        ...
        std::vector<MatrixXf> d_dt_seq_window;       // NEW: dt gradients from each timestep
    };
    ...
    RNNBackwardOutput backward(const MatrixXf &d_H_out_final_stacked)
    {
        ...
        if (T_win == 0) {
            ...
            return {{}, d_initial_h_window_empty, {}, {}};  // Added empty d_dt_seq_window
        }
        ...
        result.d_dt_seq_window.resize(T_win);  // NEW: Initialize dt gradients
        ...
        for (int t = T_win - 1; t >= 0; --t)
        {
            ...
            // NEW: Initialize dt gradient accumulator for this timestep
            MatrixXf d_dt_total_at_t = MatrixXf::Zero(cfg.batch_size, 1);
            ...
            for (int l_idx = num_layers - 1; l_idx >= 0; --l_idx)
            {
                if (cfg.enable_mask_learning && l_idx == 0) {
                    ...
                    d_dt_total_at_t += d_dt_for_layer_l_at_t;  // NEW: Accumulate dt gradient
            ...
            // NEW: Store accumulated dt gradient for this timestep
            result.d_dt_seq_window[t] = d_dt_total_at_t;
        }
